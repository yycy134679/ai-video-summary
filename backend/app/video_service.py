...
from backend.app.providers import douyin_provider, yt_dlp_provider
...
def extract_video_info(url: str) -> VideoInfo:
    ...
    return yt_dlp_provider.extract_video_info(validate_video_url(url))


def download_video(url: str, quality: Quality) -> DownloadResult:
    if douyin_provider.is_douyin_input(url):
        return douyin_provider.download_video(url, quality)
    return yt_dlp_provider.download_video(validate_video_url(url), quality)
