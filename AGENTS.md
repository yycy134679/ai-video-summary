...
- 后端：`FastAPI + yt-dlp + httpx + ffmpeg`，入口在 `backend/app/main.py`。
- 前端：`React + Vite + TypeScript + Tailwind CSS v4 + lucide-react`，入口在 `frontend/src/App.tsx`。
- 存储：无数据库；下载文件使用临时目录，请求结束后清理。
- 关键文档：`README.md`、`docs/PRD.md`、`docs/需求分析.md`、`docs/方案设计.md`、`DESIGN/DESIGN.md`、`DESIGN/首页.html`。

继续开发前先阅读 `README.md` 和 `docs/方案设计.md`，涉及产品边界或视觉改动时再读取 PRD、需求分析和设计稿。
...
- `backend/app/providers/`：平台 Provider。抖音优先走 `douyin_provider.py`，其他平台走 `yt_dlp_provider.py`。
- `backend/tests/test_video_service.py`：后端核心服务和抖音解析辅助函数测试。
...
- 新增平台能力优先封装为 Provider；站点解析优先复用 `yt-dlp`，不要轻易自研完整解析器。
